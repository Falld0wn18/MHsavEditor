...
/**
 * @Description:
 * @Name ClientResultPanel
 * @Company ZSword (C) Copyright
 * @Author JemiZhuu(周士淳)
 * @Date 2015年9月11日 下午1:52:41
 * @Version 1.0
 */
class SavDataEditPanel extends PanelBase implements ActionListener {

	public enum SavDataType {
		PersonEquipt, PersonItem, EquiptBox, ItemBox
	}
			...
			.unmodifiableMap(new LinkedHashMap<String, JSONObject>() {
				{
					...
					put("1unknow", JSONObject.parseObject("{label:'未知', length:2}"));
					put("2unknow", JSONObject.parseObject("{label:'未知', length:2}"));
					put("3unknow", JSONObject.parseObject("{label:'未知', length:2}"));
				}
			});
